...
fcheckparameters()
{
	...
	then
		exec >&2
		echo "Error: listen_port is not a number"
		fusage
		exit 1
	fi
	...
	then
		exec >&2
		echo "Error: server_port is not a number"
		fusage
		exit 1
	fi
	if ! [[ "$LISTENIP" =~ ^(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$ ]]
	then
		exec >&2
		echo "Error: listen_ip is not an ip"
		fusage
		exit 1
	fi
	if ! [[ "$SERVERIP" =~ ^(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$ ]]
	then
		exec >&2
		echo "Error: server_ip is not an ip"
		fusage
		exit 1
	fi
}
